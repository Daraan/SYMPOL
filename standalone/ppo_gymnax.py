...
def train_agent(args, trial=None, queue=None):
    ...
    for random_trial_number in range(1, args.random_trials + 1):
        ...
        obs_dim = envs.observation_space(env_params).shape[
            -1
        ]  # envs.obs_shape[-1] #envs.single_observation_space.shape[-1]
        ...
        if args.actor == "mlp" or args.actor == "stateActionDT":
            if args.action_type == "discrete":
                actor = Actor_MLP(
                    action_dim=action_dim, num_layers=args.num_layers, neurons_per_layer=args.neurons_per_layer
                )
            else:
                actor = Actor_MLP_Continuous(
                    action_dim=action_dim, num_layers=args.num_layers, neurons_per_layer=args.neurons_per_layer
                )
            ...
            actor = Actor_SDT(
                action_dim=action_dim, depth=args.depth, temperature=args.temperature, action_type=args.action_type
            )  # , temp=1)
            actor.apply = jax.jit(actor.apply)
                ...
                if args.adamW:
                    actor_state = ActorTrainState.create(
                        ...
                        grad_accum=jax.tree.map(
                            jnp.zeros_like,
                            actor.init(actor_key, jnp.array([envs.observation_space(env_params).sample(key)])),
                        ),
        ...
        def update_ppo(
            ...
            def update_epoch(carry: tuple[TrainState, TrainState, chax.PRNGKey], unused_inp):
                ...
                (actor_state, critic_state), (loss, pg_loss, v_loss, entropy_loss, approx_kl, actor_grad_accum) = (
                    jax.lax.scan(update_minibatch, (actor_state, critic_state), shuffled_storage)
                )
            ...
            (actor_state, critic_state, key), (loss, pg_loss, v_loss, entropy_loss, approx_kl, actor_grad_accum) = (
                jax.lax.scan(update_epoch, (actor_state, critic_state, key), (), length=args.n_update_epochs)
            )
            ...
            if iteration == 1 or current_eval > last_eval or global_step + batch_size >= args.total_steps:
                ...
                def fit_stateActionDT(actor_state, env_id, n_episodes, name_appendix, seed=1_000):
                    action_obs_store = ObservationActionBuffer(
                        ...
                        actions=jnp.zeros((n_steps, n_episodes) + envs.action_space().shape, dtype=jnp.int32),
                    )
                    ...
                    for episode_index in range(n_episodes):
                        if args.actor == "stateActionDT":
                            ...
                            video_path = os.path.join(
                                video_folder, run_name + "-" + "-" + env_id + str(episode_index) + ".mp4"
                            )
                            image_path = os.path.join(
                                video_folder, run_name + "-" + "-" + env_id
                            )  #  + str(episode_index))
                                        ...
                                        wandb.log(
                                            {
                                                ...
                                                + name_appendix
                                                + "_trial"
                                                + str(episode_index): wandb.Video(numpy_clip, fps=fps, format="mp4")
                                            },
                                            commit=False,
                                        )
                            ...
                            while not done and not trunc:
                                ...
                                actor_params_discrete = convert_to_discrete_tree(
                                    actor_params, args.action_type, temperature=args.temperature
                                )
                                if args.action_type == "discrete":
                            ...
                            if args.render_env and render_now:
                                ...
                                if episode_index == 0:
                                    ...
                                    if args.track:
                                        wandb.log(
                                            {
                                                "D-SDT_" + name_appendix + "_trial" + str(episode_index): wandb.Image(
                                                    image_path_plot
                                                )
                                            },
                                            commit=False,
                                        ...
                                        wandb.log(
                                            {
                                                ...
                                                + name_appendix
                                                + "_trial"
                                                + str(episode_index): wandb.Image(image_path_plot)
                                            },
                                            commit=False,
                                        )
                            ...
                            if args.n_estimators <= 5 and args.actor == "sympol" and episode_index == 0:
                                for estimator_number in range(args.n_estimators):
                                    ...
                                    image_path, node_count = plot_decision_tree(
                                        ...
                                        features_by_estimator=actor_state.indices["features_by_estimator"][
                                            estimator_number
                                        ],
                                        image_path=image_path,
                                        observation_labels=None
                                ...
                                if args.track:
                                    wandb.log(
                                        {
                                            "SDT_" + name_appendix + "_trial" + str(episode_index): wandb.Image(
                                                image_path_plot
                                            )
                                        },
                                        commit=False,
                ...
                if args.actor == "stateActionDT":
                    decision_tree = fit_stateActionDT(
                        actor_state, args.env_id, n_episodes=25, name_appendix="", seed=args.seed
                    )
                else:
                    decision_tree = None
