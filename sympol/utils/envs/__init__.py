...
try:
    from minigrid.wrappers import OneHotPartialObsWrapper, ViewSizeWrapper  # pyright: ignore[reportMissingImports]
except ModuleNotFoundError:
    print("MiniGrid not installed; skipping related wrappers")

    class RandomGoalDistShiftEnv:
        def __init__(self, *args, **kwargs):  # noqa
            raise ModuleNotFoundError("MiniGrid not installed; cannot create RandomGoalDistShiftEnv")

    class RandomGoalDistShiftEnv2:
        def __init__(self, *args, **kwargs):  # noqa
            raise ModuleNotFoundError("MiniGrid not installed; cannot create RandomGoalDistShiftEnv2")
...
if TYPE_CHECKING:
    ...
    from sympol.args import SympolCLIArgs
...
def build_env(env_id: str | _EnvSpec, n_env, view_size=3) -> "gym.vector.VectorEnv":
    ...
    if "MiniGrid" in env_id:
        env = ViewSizeWrapper(env, agent_view_size=view_size)  # pyright: ignore[reportPossiblyUnboundVariable]
        env = OneHotPartialObsWrapper(env)  # pyright: ignore[reportPossiblyUnboundVariable]
