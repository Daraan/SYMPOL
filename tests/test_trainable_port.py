...
class TestTrainable(InitRay, SympolTestHelpers, DisableLoggers, DisableGUIBreakpoints, num_cpus=4):
    ...
    @pytest.mark.basic
    def test_sympol_get_trainable_util(self):
        trainable, _ = self.get_trainable(train=False, setup_class=SympolSetup)
        self.assertEqual(trainable._setup.args.agent_type, "sympol")
    ...
    def test_1_subclass_check(self):
        ...
        TrainableClass = DefaultTrainable.define(SympolSetup.typed())
        TrainableClass2 = DefaultTrainable.define(SympolSetup.typed())
    ...
    def test_trainable_simple(self):
        def _create_trainable(self: SympolSetup):
        ...
        with mock.patch.object(SympolSetup, "_create_trainable", _create_trainable):
            with self.subTest("With parameters"):  # noqa: SIM117
                setup = SympolSetup(init_param_space=True, init_trainable=False)
    ...
    @pytest.mark.basic
    @no_parallel_envs
    @skip("Uses fcnet_hiddens which does not align with SYMPOL")
    ...
    @patch_args(
        "--batch_size", "64", "--minibatch_size", "32", "--num_envs_per_env_runner", "4", "--no_dynamic_eval_interval"
    )  # noqa: E501
    def test_train(self):
        self.TrainableClass = DefaultTrainable.define(SympolSetup.typed())
    ...
    def test_total_steps_and_iterations(self):
        with patch_args("--batch_size", "2048", "--iterations", 100), SympolSetup() as setup:
    ...
    def test_sympol_learner_added(self):
        setup = SympolSetup()
        self.assertTrue(issubclass(setup.trainable_class.setup_class.config.learner_class, JaxPPOLearnerWithLegacy))
        trainable = setup.trainable_class()
        self.assertTrue(issubclass(trainable.algorithm_config.learner_class, JaxPPOLearnerWithLegacy))

        self.assertIsInstance(trainable.algorithm.learner_group._learner, JaxPPOLearnerWithLegacy)
        trainable.stop()
    ...
    def test_learner_class_changed(self):
        ...
        self.assertIsInstance(learner, JaxPPOLearnerWithLegacy, learner)
        self.assertNotIsInstance(learner, PPOTorchLearnerWithGradientAccumulation, learner)
...
class TestClassCheckpointing(InitRay, SympolTestHelpers, DisableLoggers, num_cpus=4):
    def setUp(self):
        super().setUp()
    ...
    def test_validate_save_restore(self):
        ...
        with patch_args(
            ...
            PPOTrainable = DefaultTrainable.define(SympolSetup.typed(), fix_argv=True, use_pbar=False)
            trainable = PPOTrainable()
            assert not trainable._setup.args.render_env
    ...
    def test_restore_multiprocessing(self, cases):
        ...
        for num_env_runners in iter_cases(cases):
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                trainable_restored = DefaultTrainable.define(SympolSetup.typed()).from_checkpoint(tmpdir)
    ...
    def test_reset_config(self):
        ...
        with patch_args(
            ...
            setup1 = SympolSetup(init_param_space=True, init_trainable=False)
        ...
        with patch_args(
            ...
            setup2 = SympolSetup(init_param_space=True, init_trainable=False)
        ...
        for conf in (
            trainable2.algorithm.learner_group._learner.config,
