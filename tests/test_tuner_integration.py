...
from sympol.rllib_port import SympolSetup
...
@pytest.mark.tuner
class TestTuner(InitRay, SympolTestHelpers, DisableLoggers, num_cpus=4):
    def test_run_tune_function(self):
        ...
        with patch_args(
            ...
            with SympolSetup(init_trainable=False) as setup:
...
@pytest.mark.tuner
class TestTunerCheckpointing(InitRay, SympolTestHelpers, DisableLoggers):
    ...
    def test_checkpoint_manually(self):
        ...
        with patch_args(
            ...
            class CheckpointSetup(SympolSetup):
...
class TestReTuning(InitRay, SympolTestHelpers, DisableLoggers, num_cpus=4):
    ...
    def test_retune_with_different_config(self, cases):
        ...
        for num_env_runners in iter_cases(cases):
            with self.subTest(num_env_runners=num_env_runners):
                ...
                # do not check NUM_ENV_STEPS_PASSED_TO_LEARNER when using multiple envs per env runner
                self.assertEqual(
...
class TestReTuneScheduler(SympolTestHelpers, DisableLoggers, InitRay, num_cpus=4):
    ...
    def setUp(self):
        ...
        self.setup = SympolSetup(init_trainable=False)
...
class TestTuneWithTopTrialScheduler(SympolTestHelpers, DisableLoggers, InitRay, num_cpus=4):
    # Some tests taken from ray's testing suite
    ...
    def test_run_tune_with_top_trial_scheduler(self):
        # Switch to PPO parameters and assertions
        original_exploit = TopPBTTrialScheduler._exploit
        perturbation_interval = 128
        ...
        class CheckTrainableForTop(TrainableWithChecks):
            ...
            def step(self) -> LogMetricsDict:
                self.algorithm: PPO
                self.algorithm.config = cast("SympolPPOConfig", self.algorithm.config)
                self.algorithm_config = cast("SympolPPOConfig", self.algorithm_config)
                assert self.algorithm.config.model_config
                # PPO-specific assertions
                assert (mb := self.algorithm.config.minibatch_size) in minibatch_choices, (
                    f"minibatch_size not set correctly: {mb}"
                )
                assert self.algorithm.config.minibatch_size == mb, (
                    "minibatch_size on model_config does not match that of the config"
                )
                assert (ne := self.algorithm.config.num_epochs) in num_epochs_choices, (
                    f"num_epochs not set correctly: {ne}"
                )
                assert self.algorithm.config.num_epochs == ne, (
                    "num_epochs on model_config does not match that of the config"
                )
                assert (ec := self.algorithm.config.entropy_coeff) in entropy_coeff_choices, (
                    f"entropy_coeff not set correctly: {ec}"
                )
                assert self.algorithm.config.entropy_coeff == ec, (
                    "entropy_coeff on model_config does not match that of the config"
                )
                assert (cp := self.algorithm.config.clip_param) in clip_param_choices, (
                    f"clip_param not set correctly: {cp}"
                )
                assert (gc := self.algorithm.config.grad_clip) in grad_clip_choices, (  # noqa: RUF018
                    f"grad_clip not set correctly: {gc}"
                )
                assert self.algorithm.config.model_config["grad_clip"] == gc, (
                    "grad_clip on model_config does not match that of the config"
                )
                assert (vfcp := self.algorithm.config.vf_clip_param) in vf_clip_param_choices, (
                    f"vf_clip_param not set correctly: {vfcp}"
                )
                assert (vflc := self.algorithm.config.vf_loss_coeff) in vf_loss_coeff_choices, (
                    f"vf_loss_coeff not set correctly: {vflc}"
                )

                Tester = SympolTestHelpers()
                Tester.assertIn(self.algorithm_config.model_config["depth"], depth_choices, "depth not set correctly")
                Tester.assertIn(self.config["model_config"]["depth"], depth_choices, "depth not set correctly")
                learner_mod, runner_mod = Tester.get_modules(self)
                Tester.assertIn(
                    learner_mod.model_config["depth"], depth_choices, "learner model depth not set correctly"
                )
                Tester.assertIn(runner_mod.model_config["depth"], depth_choices, "runner model depth not set correctly")
                Tester.assertIn(
                    learner_mod.pi.config["depth"], depth_choices, "learner pi model depth not set correctly"
                )
                Tester.assertIn(runner_mod.pi.config["depth"], depth_choices, "runner pi model depth not set correctly")
                Tester.assertEqual(self.config["model_config"]["learning_rate_actor_weights"], 0.444)
                Tester.assertEqual(
                    self.algorithm_config.model_config["learning_rate_actor_weights"],
                    0.444,
                    "learning_rate_actor_weights not set correctly",
                )
                # FIXME: ATTENTION: learner._set_optimizer_lr will be called with "lr" value, which overwrites all
                # other fine-grained learning rates!
                Tester.compare_jax_learning_rate_set(self, 0.023)  # default PPO lr
                Tester.assertEqual(
                    self.algorithm.learner_group._learner.module["default_policy"].model_config["grad_clip"],
                    gc,
                    "learner model grad_clip not set correctly",
                )
                Tester.check_grad_clip_set(self, gc)

                # Check stat the steps in the states are increasing
                if self.iteration == 0:
                    states = self.algorithm.learner_group._learner.module["default_policy"].states
                    start_counts = [r[1].item() for r in optax.tree_utils.tree_get_all_with_path(states, "count")]
                    Tester.assertTrue(
                ...
                print(
                    "CHECKS PASSED for step with batch size",
                    self.algorithm_config.clip_param,
                    self.algorithm_config.grad_clip,
                )
                ...
                result["_checking_class_"] = "CheckTrainableForTop"
                logger.info(
            ...
            def get_state(self) -> dict[str, Any]:  # pyright: ignore[reportIncompatibleMethodOverride]
                state = super().get_state()
                states = self.algorithm.learner_group._learner.module["default_policy"].states
                counts = [r[1].item() for r in optax.tree_utils.tree_get_all_with_path(states, "count")]
                assert all(c == counts[0] for c in counts), f"Not all state steps are equal: {counts}"
                state["end_count"] = counts[0]
                return state

            last_count: int = 0

            def set_state(self, state: dict[str, Any] | TrainableStateDict) -> None:
                super().set_state(state)
                self.last_count = state.get("end_count")
                assert self.last_count > 0

        ray_pbt_logger.setLevel(logging.DEBUG)
        minibatch_choices = [32, 64, 128]
        num_epochs_choices = [1, 2]
        entropy_coeff_choices = [0.01, 0.02]
        clip_param_choices = [0.123, 0.321]
        grad_clip_choices = [0.111, 0.222]
        vf_clip_param_choices = [10.0, 20.0]
        vf_loss_coeff_choices = [1.0, 2.0]
        depth_choices = [2, 3]
        with patch_args(
            "--algorithm", "ppo",
            "--tune", "clip_param", "grad_clip",
            "--num_samples", 1,
            "--num_jobs", 4,
            ...
            Setup = SetupWithCheck(CheckTrainableForTop, SympolPBTSetup)
            with Setup(
                config_files=[],
            ) as setup:
                setup.config.reporting(min_sample_timesteps_per_iteration=min(batch_sizes))
            setup.param_space["clip_param"] = tune.grid_search(clip_param_choices)
            setup.param_space["grad_clip"] = tune.grid_search(grad_clip_choices)
            ...
            setup.args.command.set_hyperparam_mutations(
                {
                    ...
                    "minibatch_size": CyclicMutation(minibatch_choices),
                    "num_epochs": CyclicMutation(num_epochs_choices),
                    "entropy_coeff": CyclicMutation(entropy_coeff_choices),
                    "grad_clip": KeepMutation(),
                    "vf_clip_param": CyclicMutation(vf_clip_param_choices),
                    "vf_loss_coeff": CyclicMutation(vf_loss_coeff_choices),
                    "clip_param": KeepMutation(),
                    "model_config": {
                        "learning_rate_actor_weights": KeepMutation(0.444),
                        "depth": CyclicMutation(depth_choices),
                    },
                    "lr": KeepMutation(0.023),
                }
            )
            results = run_tune(setup)
            raise_tune_errors(results)
            self.assertTrue(all(result.metrics["_checking_class_"] == "CheckTrainableForTop" for result in results))
            # There are 4 trials, 3 will be exploited at each perturbation interval and we have 2 intervals
            self.assertEqual(num_exploits, (len(clip_param_choices) * len(grad_clip_choices) - 1) * 2)
            # Check that at most one race condition happened
            self.assertLessEqual(race_conditions, 1)
            self.assertTrue(all(r.config["train_batch_size_per_learner"] in batch_sizes for r in results))
            self.assertTrue(all(r.config["model_config"]["depth"] in depth_choices for r in results))
            self.assertTrue(all(r.config["clip_param"] in clip_param_choices for r in results))
            self.assertTrue(all(r.config["minibatch_size"] in minibatch_choices for r in results))
            self.assertTrue(all(r.config["num_epochs"] in num_epochs_choices for r in results))
            self.assertTrue(all(r.config["entropy_coeff"] in entropy_coeff_choices for r in results))
