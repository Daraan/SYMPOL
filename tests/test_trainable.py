...
class TestTraining(SympolSetupDefaults):
    def setUp(self) -> None:
        ...
        with patch_args("--agent_type", "sympol"):
            self._SETUP = SympolSetup(init_param_space=False, init_config=False, init_trainable=False)
            ...
            self._SETUP.create_config()
            with self._SETUP.open_config() as config:
                self._ALGORITHM_CONFIG = self._SETUP.config  # type: ignore[assignment]
                config.training(
    ...
    def test_trainable(self):
        ...
        with self.subTest("With parameters"), patch_args(*sys.argv[1:], "--hostname_selector", socket.gethostname()):
            ...
            trainable = setup.trainable_class(params)
            _result = trainable.train()
