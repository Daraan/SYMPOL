...
from sympol._test_utils import SympolSetupDefaults, SympolTestHelpers, sympol_patch_args
...
class TestSetupClasses(InitRay, SympolSetupDefaults, num_cpus=4):
    @pytest.mark.basic
    @mock_trainable_algorithm
    def test_basic(self):
        with patch_args():
    ...
    def test_project_name_substitution(self):
        setup = SympolSetup(init_trainable=False, init_param_space=False, init_config=False)
        setup.PROJECT = "Test-<agent_type>-<env_type>"
        self.assertEqual(setup.project.rstrip("-v0123456789"), "Test-sympol-CartPole")
    ...
    def test_dynamic_param_spaces(self):
        ...
        with patch_args("--tune", "dynamic_buffer"):
            ...
            with self.assertRaises(SystemExit) as context:
                SympolSetup().create_param_space()
        ...
        for param in hint_lists:
            with (
                ...
                setup = SympolSetup(init_trainable=False)
    ...
    def test_seeding_env(self, cases):
        for num_env_runners in iter_cases(cases):
            with (
                ...
                SympolSetup(init_trainable=False) as setup,
            ):
    ...
    def test_cfg_loading(self):
        with tempfile.NamedTemporaryFile("w+") as f, patch_args("-cfg", f.name):
            args = ["--tag:sympol", "--tag:sympol_depth=3", "--agent_type sympol", "--depth 3"]
            f.write("\n".join(args))
            f.flush()
            setup = SympolSetup(init_param_space=False)
            self.assertListEqual(["\n".join(args)], setup.parser.args_from_configs)
            self.assertEqual(setup.args.depth, 3)
            ...
            self.assertEqual(module.model_config["depth"], 3)  # pyright: ignore[reportIndexIssue]
            self.assertIn("depth=3", str(module))
            ...
            self.assertEqual(spec_module.model_config["depth"], 3)  # pyright: ignore[reportIndexIssue]
            self.assertIn("depth=3", str(spec_module))
    ...
    def test_lr_loading(self):
        with patch_args("--lr", "0.123"):
            ...
            actor_opt_state = learner.get_jax_states()["default_policy"]["actor"].opt_state
            lr = actor_opt_state[1][0]["estimator_weights"][0].hyperparams["learning_rate"]
            self.assertEqual(lr, 0.123)
        return
        with patch_args("--lr", "[[0, 0.111], [64, 0.333]]", "--depth", 2, "--batch_size", 32):
            ...
            assert learner
            # Check learning rate in JAX opt_state (PartitionState is at index 1)
            actor_opt_state = learner.get_jax_states()["default_policy"]["actor"].opt_state
            lr = actor_opt_state[1][0]["estimator_weights"][0].hyperparams["learning_rate"]
            self.assertAlmostEqual(lr, 0.111)
            ...
            for i in range(1, 5):
                if i == 1:
                    lr = (
                        learner.get_jax_states()["default_policy"]["actor"]
                        .opt_state[1][0]["estimator_weights"][0]
                ...
                if i == 1:
                    ...
                    self.assertAlmostEqual(lr, 0.222)
                if i >= 2:
                    lr = (
                        learner.get_jax_states()["default_policy"]["actor"]
                        .opt_state[1][0]["estimator_weights"][0]
                    ...
                    self.assertAlmostEqual(lr, 0.333)
...
@unittest.skip("DQN not available for SYMPOL")
...
class TestAlgorithm(InitRay, SympolSetupDefaults, num_cpus=4):
    def setUp(self):
        super().setUp()
    ...
    def test_no_max_iteration_stopper_when_tuning(self):
        ...
        # FIXME: eval_return_ema is not added
    ...
    def test_model_config_saver_callback_creates_json(self):
        ...
        # Use SympolSetup to build a real Algorithm
        with (
            patch_args(
                "--depth", "2",
                "-it", 1,
        ...
        self.assertIn("depth=2", data["architecture"]["summary_str"])
        # Value Function:
        self.assertIn("neurons_per_layer = 123", data["architecture"]["summary_str"])
        os.remove(out_path)
